        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                try {
                    ...
                    // Get Base Tables
                    $baseTables = [];
                    $stmt_bt = $conn->prepare("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_TYPE = 'BASE TABLE'");
                    $dbName = DB_NAME;
                    $stmt_bt->bind_param("s", $dbName);
                    $stmt_bt->execute();
                    $res_bt = $stmt_bt->get_result();
                    while($row = $res_bt->fetch_row()) $baseTables[] = $row[0];
                    $stmt_bt->close();

                    // Get Views
                    $views = [];
                    $stmt_v = $conn->prepare("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_TYPE = 'VIEW'");
                    $stmt_v->bind_param("s", $dbName);
                    $stmt_v->execute();
                    $res_v = $stmt_v->get_result();
                    while($row = $res_v->fetch_row()) $views[] = $row[0];
                    $stmt_v->close();
                    ...
                    // Process Base Tables (Structure + Data)
