 ...
 * Archives the blotter record to the archive table, then deletes from blotter_records.
 * This ensures deleted blotters are recoverable from the Archive page.
 */
...
try {
    ...
    // Convenience fields for quick display in archive.php viewDetails()
    ...
    if (isset($_SESSION['username'])) {
        try {
            ...
            $log_desc   = "Moved blotter record to archive: $recordNumber";
    ...
    echo json_encode(['success' => true, 'message' => 'Blotter record moved to archive successfully']);

} catch (Exception $e) {
    $conn->rollback();
    error_log("delete_blotter_record.php error: " . $e->getMessage());
