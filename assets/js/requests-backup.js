...
(function() {
    ...
    function loadCertificates() {
        fetch('model/get_certificates.php')
    ...
    function loadResidents() {
        ...
        fetch('model/search_residents.php')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    function generatePreview() {
        ...
        // Fetch preview data
        fetch(`model/get_certificate_preview.php?resident_id=${selectedResidentId}&certificate_id=${selectedCertificateId}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    if (data.use_html) {
                        ...
                        renderPDFPreview(data.pdf_path, data.fields, data.resident_data);
                    }
                } else {
    ...
    function handlePrint() {
        ...
        // Save to database
        fetch('model/save_certificate_request.php', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
